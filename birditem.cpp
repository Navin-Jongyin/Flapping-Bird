...
{
    setPixmap(pixmap);
    QTimer * birdWingsTimer = new QTimer;
...
void BirdItem::updatePixmap()
{
    if(wingPosition == WingPosition::Middle)
    {
        if(wingDirection)
        {
            setPixmap(QPixmap(":/Images/BirdWingUp.png"));
            wingPosition = WingPosition::Up;
            wingDirection = 0;
        }
        else
        {
            setPixmap(QPixmap(":/Images/BirdWingDown.png"));
    ...
    {
        setPixmap(QPixmap(":/Images/BirdWingNormal.png"));
